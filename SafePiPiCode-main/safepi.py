  ...
  while True:
    ...
    if current_stateS1 != previous_stateS1:
      ...
      if current_stateS1 == False:
        ...
        print(status)
        requests.put('http://34.94.122.160:8080/api/v1/sensors/Pi0/Sen0', data=status, headers=headers) # sends data to server. IP address and port number will have to be replaced before use.
        time.sleep(.5)
      else:
    ...
    if current_stateS2 != previous_stateS2:
      ...
      if current_stateS2 == False:
        ...
        status = '{"status": "Closed", "timestamp": "' + time_stamp + '"}'
        print(status)
        requests.put('http://34.94.122.160:8080/api/v1/sensors/Pi0/Sen1', data=status, headers=headers) # sends data to server. IP address and port number will have to be replaced before use.
        time.sleep(.5)
      else:
